...
struct Actividad  // ESTRUCTURA PRINCIPAL . LUGAR DONDE SE ALMACENA LA INFORMACIÓN DEL FICHERO
{
    int anio; 
    int edad; 
    int mes; 
...
int main()  // MAIN. Donde se aloja toda la informacion y estructura del programa
{
    ...
    // INICIO DE SESION / REGISTRO CON USUARIO Y CONTRASEÑA
    char usuario[50];
	char contrasena[50];
    ...
    // BUCLE DO WHILE de validacion de las condiciones para el nombre y las contraseñas.
    do 
	{
        ...
        if (SoloLetras(usuario) == 1 && SoloLetras(contrasena) == 1) // Comprobamos con la funcion SoloLetras que la contraseña y el usuario no contienen caracteres especiales. 
    	{
	...
	do {
        ...
        switch(opcion)
		{
			...
			case 9:
			{
				...
				// DISEÑAMOS EL CONTENIDO DEL ARCHIVO
				fprintf(fsalida, "====================================================================\n");
			...
			case 10:
			{
			    ...
			    do
			    {
			    	// DISEÑAMOS LO QUE QUEREMOS QUE TENGA EL CERTIFICADO
